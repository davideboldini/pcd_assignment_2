...
public class GuiController implements Runnable, ModelObserver{
    ...
    public GuiController(final FileMonitor fileMonitor, final IntervalMonitor intervalMonitor, final ThreadMonitor threadMonitor){
        ...
        try {
            ...
            new Thread(() -> {
                try {
                    ...
                    this.controller = new ControllerThread(threadMonitor);
                    Thread tDir = new Thread(controller);

                    this.controller.addThread(new Thread(new DirectoryThread(D, fileMonitor, intervalMonitor, threadMonitor, controller)));
                    tDir.start();
                    tDir.join();
                    this.running = false;
