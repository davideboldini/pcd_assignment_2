	...
	public void start(){
		...
		eventBus.consumer("gui-update-topic", (Message<MessageUpdate> message) -> {
			...
			if (mex.getTypeMessage().equals(MessageType.FILE_LENGTH)) {
				TreeSet<Pair<File, Long>> fileLengthTree = new TreeSet<>(mex.getFileLengthMap());
				this.setFileLengthGui(fileLengthTree);
			} else {
				HashMap<Pair<Integer, Integer>, Integer> intervalMap = new HashMap<>(mex.getIntervalMap());
	...
	private void setFileLengthGui(final TreeSet<Pair<File, Long>> fileTree){
		try {
			SwingUtilities.invokeLater(() -> {
				if (fileTree.size() > N) {
					textAreaFileLength.setText(printer.printFileLengthGui(fileTree, N));}
			});
		} catch (Exception ex) {
			ex.printStackTrace();
		}
